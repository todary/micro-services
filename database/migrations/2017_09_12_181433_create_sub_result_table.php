...
class CreateSubResultTable extends Migration {
	...
	public function up()
	{
		Schema::create('sub_result', function(Blueprint $table)
		{
			...
			$table->bigInteger('entity_id')->unsigned()->nullable()->index('entity');
			$table->bigInteger('report_id')->unsigned()->index('fk_sub_result_1_idx');
			$table->bigInteger('result_id')->unsigned()->index('fk_sub_result_2_idx');
			...
			$table->unique(['report_id','unique_content'], 'report_id');
		});
	}
