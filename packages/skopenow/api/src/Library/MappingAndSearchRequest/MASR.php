...
namespace Skopenow\Api\Library\MappingAndSearchRequest;
...
class MASR
{
    ...
    public function mappingRequest($data)
    {
        $requestArray = [];
        foreach ($this->requestArray as $index => $values) {
            if (is_array($values) && !empty($values)) {
                foreach ($values as $key => $value) {
                    if (isset($data[$key])) {
                        $requestArray[$index][$key] = $data[$key];
                    }
                }
            } else {
                if (!is_array($values)) {
                    if (isset($data[$values])) {
                        $requestArray[$values] = $data[$values];
                    }
                } else {
                    if (isset($data[$index])) {
                        $requestArray[$index] = $data[$index];
                    }
                }
            }
        }
    ...
    public function getRequestArray($option, $report_id, $limit = 999, $offset = 0)
    {

        $this->requestArray['id'] = encryptID($report_id);
        $this->requestArray['limit'] = $limit;
        $this->requestArray['offset'] = $offset;
    ...
    {
        $summary = self::getSummary(decryptID($id));
        return $summary;
    }
    ...
    public function removeHideData($hide, $resultfinal)
    {
        if (isset($hide)) {
            foreach ($hide as $key) {
                switch ($key) {
                    ...
                    case 'photos':
                        ...
                        foreach ($resultfinal['results'] as $res_key => $res_value) {
                            foreach ($resultfinal['results'][$res_key]['details'] as $key => $value) {
                                unset($resultfinal['results'][$res_key]['details'][$key]['image']);
                            }
                        }
                        break;
                    case 'urls':
                        ...
                        foreach ($resultfinal['results'] as $res_key => $res_value) {
                            foreach ($resultfinal['results'][$res_key]['details'] as $key => $value) {
                                unset($resultfinal['results'][$res_key]['details'][$key]['url']);
                            }
                        }
                        break;
                    case 'tags':
                        foreach ($resultfinal['results'] as $res_key => $res_value) {
                            foreach ($resultfinal['results'][$res_key]['details'] as $key => $value) {
                                unset($resultfinal['results'][$res_key]['details'][$key]['tags']);
                            }
                        }
                        break;
                    case 'ip':
                        foreach ($resultfinal['results'] as $res_key => $res_value) {
                            unset($resultfinal['results'][$res_key]['ip']);
                        }
                        break;
                }
            }
