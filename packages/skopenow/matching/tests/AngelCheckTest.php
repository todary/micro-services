...
class AngelCheckTest extends TestCase
{
    /** @test */
    public function should_return_matched_data_from_angel_check()
    {
        $url = 'https://angel.co/kevin-leung';
        $comb = ['id' => 10];
        $check = new AngelCheck($url, new \ArrayIterator(['id' => 59954]), $comb);
                $status = $check->check();
        $expected = [
            'name' => [
                'status' => true,
    ...
    /** @test */
    public function should_return_unmatched_data_from_angel_check()
    {
        $url = 'https://angel.co/kevin-leung';
        $comb = ['id' => 10];
        $check = new AngelCheck($url, new \ArrayIterator(['id' => 59954]), $comb);
