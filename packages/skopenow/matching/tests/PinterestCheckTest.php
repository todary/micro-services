...
class PinterestCheckTest extends TestCase
{
    /** @test */
    public function should_return_matched_data_from_pinterest_check()
    {
        $url = 'https://www.pinterest.co.uk/jennifer';
        $comb = ['id' => 10];
        $check = new PinterestCheck($url, new \ArrayIterator(['id' => 59954]), $comb);
        $status = $check->check();
        $expected = [
            'name' => [
                'status' => true,
    ...
    /** @test */
    public function should_return_unmatched_data_from_pinterest_check()
    {
