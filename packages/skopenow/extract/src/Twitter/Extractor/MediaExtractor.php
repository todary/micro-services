...
class MediaExtractor implements ExtractorInterface
{
    ...
    public function Process() : array
    {
        ...
        if(!empty($html)) {
            ...
            $pattern = "/data-permalink-path=\"(.+?)\".*?data-image-url=\"(.+?)\"/s";
            $isMatch = preg_match_all($pattern, $page, $match);
            if($isMatch) {
                foreach ($match[1] as $key => $tweetUrl) {
                    $permalinkUrl  = "http://twitter.com" . $tweetUrl;
                    $tweetId = explode("/", $tweetUrl)[3];
                    $imageUrl = array("imageUrl"=> $match[2][$key]);
                    $imagesTweets[] = array('tweet_id' => $tweetId, 'permalinkUrl' => $permalinkUrl, 'imageUrl' => $imageUrl);
                }
            }
        }
        return array("imagestweets" => $imagesTweets, "vidoetweets" => $videoTweets);
    }
