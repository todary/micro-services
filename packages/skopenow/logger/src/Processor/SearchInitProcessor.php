...
class SearchInitProcessor extends AbstractProcessor implements ProcessorInterface
{
	/**
	 *
	...
	{
		if(empty($data['names']) && empty($data['locations']) && empty($data['addresses']) 
				&& empty($data['emails']) && empty($data['phones']) && empty($data['username']) 
		){
			$this->validation_notes['required'] = "You must enter name & location , address , email , phone , username " ;
		}
