...
class FormatterProcessor extends AbstractProcessor implements ProcessorInterface
{
    /**
     *
    ...
    public function process(array $state, array $data) :bool
    {
        ...
        $this->setDataModelAttribute("document", "formatter");
    ...
    {
        if (empty($data["input"]) || empty($data["format_type"])) {
            $this->validation_notes['required'] = "Inputs or format type can't be empty" ;
        }
        return true;
    }
