...
class TripadvisorProfileInfo extends SocialProfileInfoAbstraction implements ProfileInfoInterface
{
    private $source = [
        'name' => "/nameText[^>]*>([^<]*)<\\/sp/is",
        'image' => "/avatarUrl[\"']\\W*src=[\"']([^'\"]*)['\"]/is",
        'location' => "/hometown['\"]><p>([^<]*)</is"
    ];

    public function getProfileInfo(string $url, array $htmlContent)
    {
        \Log::info('URLInfo: getProfileInfo from tripadvisor');
