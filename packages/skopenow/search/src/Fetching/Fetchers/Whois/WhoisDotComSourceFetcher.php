...
class WhoisDotComSourceFetcher implements DomainFetcherInterface
{
    ...
    public function processResponse(string $body, $criteria)
    {
        ...
        preg_match('/REGISTRANT CONTACT(.+)df-block/i', $content, $registrantContent);

        preg_match("/ADMINISTRATIVE CONTACT(.+)df-block/i", $content, $adminContent);
    ...
    {
        $ex = '/Name:<\/div><div class="df-value">(.*?)<\/div>/i';
    ...
    {
        $ex = '/Phone:<\/div><div class="df-value">(.*?)<\/div>/i';
        preg_match($ex, $content, $matches);
        if (isset($matches[1])) {
            return $matches[1];
        }
